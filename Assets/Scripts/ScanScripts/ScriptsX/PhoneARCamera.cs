...
public class PhoneARCamera : MonoBehaviour
{
    ...
    public void OnScanExit() // does same thing as OnRefresh() but specifically is for clearing all objects before switching to Journal Screen
    {
        if (boxOutlines != null) //otherwise if theres no ARCamera then boxOutlines is null reference and app crashes
        {
            Debug.Log("DEBUG: onScanExit, removing anchors and boundingboxes");
            localization = false;
            staticNum = 0;
            // clear boubding box containers
            boxSavedOutlines.Clear();
            ...
            AnchorCreator anchorCreator = FindObjectOfType<AnchorCreator>();
            anchorCreator.RemoveAllAnchors();
            GameObject.Find("Detector Yolo2-tiny").SetActive(false);
            if (m_CameraManager != null)
            {
                m_CameraManager.frameReceived -= OnCameraFrameReceived;
            }
